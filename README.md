...
Fundacion completada y demostracion funcional disponible. Ya estan definidos y versionados:
...
- interfaz funcional con panel, empleados, horarios, asistencia y terminal;
- fichaje demo con CLOCK_IN, BREAK_START, BREAK_END y CLOCK_OUT;
- base i18n para español, polski y English;
- clientes Supabase para navegador y servidor.

La interfaz funciona en modo demo y conserva datos solo durante la sesion abierta. Este aislamiento es intencional: no se presenta almacenamiento del navegador como produccion. Para persistencia real deben aplicarse las migraciones y configurarse las variables Supabase.
...
El proyecto usa React + Vite y contiene `vercel.json`. Vercel debe ejecutar `pnpm run build` y publicar `dist`. No requiere adaptadores de Cloudflare, OpenAI Sites ni imports desde `build/`.
